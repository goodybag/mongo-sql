...
module.exports = {
  ...
  $equals: function(column, value, values, collection){
    return utils.quoteColumn(column, collection) + ' = ' + value;
  }
...
, $null: function(column, value, values, collection){
    return utils.quoteColumn(column, collection) + ' is null';
  }
...
, $notNull: function(column, value, values, collection){
    return utils.quoteColumn(column, collection) + ' is not null';
  }
...
, $like: function(column, value, values, collection){
    return utils.quoteColumn(column, collection) + ' like ' + value;
  }
...
, $in: function(column, set, values, collection){
    ...
    if (typeof set === "function")
      return utils.quoteColumn(column, collection) + ' in (' + set(values) + ')';

    if (Array.isArray(set))
      return utils.quoteColumn(column, collection) + ' in (' + set.join(', ') + ')';

    return utils.quoteColumn(column, collection) + ' in (' + set + ')';
...
, $nin: function(column, set, values, collection){
    ...
    // So pass it the current set of values to build upon
    if (typeof set === "function")
      return utils.quoteColumn(column, collection) + ' not in (' + set(values) + ')';

    if (Array.isArray(set))
      return utils.quoteColumn(column, collection) + ' not in (' + set.join(', ') + ')';

    return utils.quoteColumn(column, collection) + ' not in (' + set + ')';
