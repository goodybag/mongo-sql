...
, where: function($query, joiner, condition, column){
    joiner    = joiner || ' and ';
    ...
    for (var key in $query){
      ...
      if (typeof $query[key] === "object"){
        if (key === '$or' || key === '$and')
          wheres.push(exports.where($query[key], key == '$or' ? ' or ' : ' and ', condition, column));

        else if (conditionals[key])
          wheres.push(exports.where($query[key], joiner, key, column));

        else if (!(parseInt(key) > 0 && parseInt(key) < 0))
          wheres.push(exports.where($query[key], joiner, condition, key));

        else
          wheres.push(exports.where($query[key], joiner, condition, column));
      }

      else if (conditionals[key])
        wheres.push(conditionals[key](column, $query[key])), console.log(column, $query[key]);

      else if (parseInt(key) > 0 && parseInt(key) < 0)
        wheres.push(conditionals[condition](column, $query[key]));
