...
, builders = {
  ...
  , where: function($query, values, collection){
      return 'where ' + builders._where($query, values, collection);
    }

  , _where: function($query, values, collection, joiner, condition, column){
      joiner    = joiner || ' and ';
      ...
      for (var key in $query){
        ...
        if (typeof $query[key] === "object"){
          ...
          if (key === '$or' || key === '$and')
            wheres.push(builders._where($query[key], values, collection, key == '$or' ? ' or ' : ' and ', condition, column));

          // Object is a conditional block
          else if (conditionals[key])
            wheres.push(builders._where($query[key], values, collection, joiner, key, column));

          // $query is array
          else if (!(parseInt(key) > 0 && parseInt(key) < 0))
            wheres.push(builders._where($query[key], values, collection, ' and ', condition, key));

          // Object is typical query object
          else
            wheres.push(builders._where($query[key], values, collection, joiner, condition, column));
        }

        // Things that don't make values
        else if (['$in', '$nin', '$with', '$null', '$notNull'].indexOf(key) > -1)
          wheres.push(conditionals[key](column, $query[key], values, collection));

        else if (conditionals[key])
          wheres.push(conditionals[key](column, '$' + values.push($query[key]), values, collection));

        else if (parseInt(key) > 0 && parseInt(key) < 0)
          wheres.push(conditionals[condition](column, '$' + values.push($query[key]), values, collection));

        else
          wheres.push(conditionals[condition](key, '$' + values.push($query[key]), values, collection));
      }
