...
, where: function($query, joiner, condition, column){
    ...
    for (var key in $query){
      ...
      if (typeof $query[key] === "object"){
        if (key === '$or' || key === '$and')
          wheres.push(module.exports.where($query[key], key == '$or' ? ' or ' : ' and ', condition, column));

        else if (conditionals[key])
          wheres.push(module.exports.where($query[key], joiner, key, column));

        else if (!(parseInt(key) > 0 && parseInt(key) < 0))
          wheres.push(module.exports.where($query[key], ' and ', condition, key));

        else
          wheres.push(module.exports.where($query[key], joiner, condition, column));
      }

      else if (conditionals[key])
        wheres.push(conditionals[key](column, $query[key]));
