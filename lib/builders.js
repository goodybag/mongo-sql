...
, where: function($query, values, joiner, condition, column){
    ...
    for (var key in $query){
      ...
      if (typeof $query[key] === "object"){
        ...
        wheres.push(conditionals[key](column, '$' + values.push($query[key])));

      else if (parseInt(key) > 0 && parseInt(key) < 0)
        wheres.push(conditionals[condition](column, '$' + values.push($query[key])));

      else 
        wheres.push(conditionals[condition](key, '$' + values.push($query[key])));
    }
