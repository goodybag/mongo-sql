...
, valueBehaviors  = require('./value-behaviors')
...
, builders = {
  ...
  , where: function($query, values, collection){
    ...
    // Conditions iterative - new version of conditions
  , conditionsI: function($query, collection, values){
      ...
      while (current = queue.shift()){
        ...
        for (var key in current){
          ...
          if (typeof current[key] == 'object' && !valueBehaviors[key]){
          ...
          // Key is custom value behavior
          else if (valueBehaviors[key]){
            conditions.push(
              conditionals[condition](
                utils.quoteColumn(column, collection)
                // If using values, use $ syntax. No values means val is a column
              , values ? valueBehaviors[key](
                  utils.quoteColumn(column, collection), current[key], values, collection
    ...
    // Conditions recursive: Deprecated
